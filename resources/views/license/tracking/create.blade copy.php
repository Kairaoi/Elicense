...
function addSpeciesRow() {
    ...
    // If agent is already selected, load species
    const agentId = $('#agent_id').val();
    if (agentId) {
        updateSpeciesDropdowns(agentId);
    }
...
function updateSpeciesDropdowns(agentId) {
    if (!agentId) {
        $('.species-select').each(function() {
            $(this).empty().append('<option value="">Select Species</option>');
            $(this).trigger('change');
        });
        return;
    }

    console.log(`Fetching species for agent ID: ${agentId}`); // Debugging log

    $.ajax({
        url: `/api/agents/${agentId}/species`,
        method: 'GET',
        success: function(species) {
            console.log('Species data received:', species); // Debugging log

            $('.species-select').each(function() {
                const select = $(this);
                select.empty().append('<option value="">Select Species</option>');
                species.forEach(item => select.append(new Option(item.name, item.id)));
                select.trigger('change');
            });
        },
        error: function(xhr) {
            console.error('Error loading species:', xhr); // Debugging log
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: xhr.responseJSON.message || 'Failed to load species for this agent'
            });
        }
    });
}
