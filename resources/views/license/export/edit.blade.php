...
@section('content')
<div class="container">
    <h1>Edit Export Declaration</h1>

    <form action="{{ route('export.declarations.update', $exportDeclaration->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <select name="applicant_id" id="applicant_id" class="form-control @error('applicant_id') is-invalid @enderror">
                        ...
                        @foreach($applicants as $id => $applicant)
                            <option value="{{ $id }}" {{ old('applicant_id', $exportDeclaration->applicant_id) == $id ? 'selected' : '' }}>
                                {{ $applicant }} 
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="shipment_date" id="shipment_date" class="form-control @error('shipment_date') is-invalid @enderror" value="{{ old('shipment_date', $exportDeclaration->shipment_date instanceof \Carbon\Carbon ? $exportDeclaration->shipment_date->format('Y-m-d') : $exportDeclaration->shipment_date) }}">
                    @error('shipment_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="export_destination" id="export_destination" class="form-control @error('export_destination') is-invalid @enderror" value="{{ old('export_destination', $exportDeclaration->export_destination) }}" placeholder="Enter export destination">
        ...
        <div id="species-container">
            @foreach($exportDeclaration->species as $index => $species)
                <div class="species-row row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="species_id_{{ $index + 1 }}">Species</label>
                            <select name="species[{{ $index + 1 }}][species_id]" id="species_id_{{ $index + 1 }}" class="form-control species-select">
                                ...
                                @foreach($speciesList as $id => $speciesInfo)
                                    ...
                                    @endphp
                                    <option value="{{ $id }}" data-unit-price="{{ $unitPrice }}" {{ $species->species_id == $id ? 'selected' : '' }}>
                                        {{ $speciesInfo }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="volume_kg_{{ $index + 1 }}">Volume (kg)</label>
                            <input type="number" name="species[{{ $index + 1 }}][volume_kg]" id="volume_kg_{{ $index + 1 }}" class="form-control" step="0.01" value="{{ $species->volume_kg }}" />
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                            <label for="under_size_volume_kg_{{ $index + 1 }}">Under-size Volume (kg)</label>
                            <input type="number" name="species[{{ $index + 1 }}][under_size_volume_kg]" id="under_size_volume_kg_{{ $index + 1 }}" class="form-control" step="0.01" value="{{ $species->under_size_volume_kg }}" />
                        </div>
                    </div>
                    <input type="hidden" name="species[{{ $index + 1 }}][unit_price]" id="unit_price_{{ $index + 1 }}" value="{{ $species->fee_per_kg }}" />
                </div>
            @endforeach
        </div>
...
<script>
    let speciesCount = {{ count($exportDeclaration->species) }};
